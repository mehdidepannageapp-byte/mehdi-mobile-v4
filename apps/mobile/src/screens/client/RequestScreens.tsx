...
import type { IssueType, PaymentMethod, Place, RootStackParamList, Vehicle } from '../../types';
...
export function IssueChoiceScreen({ navigation }: Props<'IssueChoice'>) {
  ...
  return <AppScreen><Header title="Que se passe-t-il ?" subtitle="Étape 1 sur 4" onBack={() => navigation.goBack()} />{issues.map((issue) => <OptionCard key={issue.value} icon={issue.icon} title={issue.title} selected={draft.issueType === issue.value} onPress={() => updateDraft({ issueType: issue.value })} tone={issue.value === 'ACCIDENT' ? 'red' : 'yellow'} />)}
...
function AddressSearch({ label, value, onSelect, onClear, locate }: { label: string; value?: Place; onSelect: (place: Place) => void; onClear: () => void; locate?: () => void }) {
...
export function RouteChoiceScreen({ navigation }: Props<'RouteChoice'>) {
  ...
  return <AppScreen><Header title="Trajet de la moto" subtitle="Étape 3 sur 4" onBack={() => navigation.goBack()} /><AddressSearch label="Adresse de prise en charge" value={draft.pickup} onSelect={(pickup) => updateDraft({ pickup, distanceKm: 0 })} onClear={() => updateDraft({ pickup: undefined, distanceKm: 0 })} locate={locate} />{locating ? <Text style={ui.muted}>Recherche de votre position…</Text> : null}<AddressSearch label="Adresse de destination" value={draft.destination} onSelect={(destination) => updateDraft({ destination, distanceKm: 0 })} onClear={() => updateDraft({ destination: undefined, distanceKm: 0 })} />
    ...
    <PrimaryButton title="Choisir la date" onPress={() => { if (distanceKm !== null) { updateDraft({ distanceKm }); navigation.navigate('Schedule'); } }} disabled={distanceKm === null} />
  </AppScreen>;
}
...
export function QuoteScreen({ navigation }: Props<'Quote'>) {
  ...
  useEffect(() => {
    ...
    api.estimate({ issueType: draft.issueType, distanceKm, scheduledFor: draft.scheduledFor?.toISOString() })
      .then((result) => {
  ...
  async function pay() {
    if (loading) return;
    if (amount === null || !draft.issueType || !draft.pickup || !draft.destination || invalidRoute) {
    ...
    try {
      ...
      const booking = await api.createBooking({ issueType: draft.issueType, issueDescription: draft.issueDescription, pickup: draft.pickup, destination: draft.destination, distanceKm, vehicleId: vehicle.id, scheduledFor: draft.scheduledFor?.toISOString(), paymentMethod });
