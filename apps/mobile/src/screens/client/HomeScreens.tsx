...
export function ClientProfileScreen({ navigation }: Props<'ClientProfile'>) {
  ...
  return <AppScreen><Header title="Mon profil" subtitle={user?.phone} />
    ...
    <SectionTitle>Mon compte</SectionTitle><OptionCard icon="bicycle" title="Mes motos" subtitle="Ajouter ou modifier une moto" onPress={() => Alert.alert('Mes motos', 'Une moto peut également être ajoutée pendant la demande.')} /><OptionCard icon="notifications" title="Notifications" subtitle="Missions et suivi de l’intervention" onPress={() => undefined} /><OptionCard icon="help-circle" title="Support et FAQ" onPress={() => navigation.navigate('Support')} />
