...
export const api = {
  ...
  updateVehicle: (id: string, data: object) => request<import('../types').Vehicle>(`/vehicles/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  ...
  bookingsConfig: () => request<{ pricing: { baseByIssueCents: Record<string, number>; perKmCents: number; nightSurchargeRate: number; weekendSurchargeRate: number } }>('/bookings/config'),
  ...
  createIncident: (id: string, type: string, description?: string) => request<import('../types').Incident>(`/bookings/${id}/incidents`, { method: 'POST', body: JSON.stringify({ type, description }) }),
  notificationPreferences: () => request<{ pushEnabled: boolean }>('/auth/me/notifications'),
  updateNotificationPreferences: (pushEnabled: boolean) => request<{ pushEnabled: boolean }>('/auth/me/notifications', { method: 'PATCH', body: JSON.stringify({ pushEnabled }) }),
  garages: () => request<import('../types').Garage[]>('/garages'),
  convertToTransport: (id: string, destination: object, distanceKm: number) => request<import('../types').Booking>(`/bookings/${id}/convert-to-transport`, { method: 'POST', body: JSON.stringify({ destination, distanceKm }) }),
  requestAppointmentChange: (id: string, proposedFor: string) => request<import('../types').AppointmentChangeRequest>(`/bookings/${id}/appointment-change`, { method: 'POST', body: JSON.stringify({ proposedFor }) }),
  acceptAppointmentChange: (id: string, changeId: string) => request<import('../types').Booking>(`/bookings/${id}/appointment-change/${changeId}/accept`, { method: 'POST' }),
  rejectAppointmentChange: (id: string, changeId: string) => request(`/bookings/${id}/appointment-change/${changeId}/reject`, { method: 'POST' }),
  driverAppointmentChanges: () => request<import('../types').Booking[]>('/driver/appointment-changes'),
  applySurcharge: (id: string, category: import('../types').SurchargeCategory, extraKm?: number) => request<import('../types').Surcharge>(`/bookings/${id}/surcharges`, { method: 'POST', body: JSON.stringify({ category, extraKm }) }),
  contactAttempt: (id: string, method: 'CALL' | 'MESSAGE', note?: string) => request<import('../types').ContactAttempt>(`/bookings/${id}/contact-attempts`, { method: 'POST', body: JSON.stringify({ method, note }) }),
  declareAbsence: (id: string, reason: string) => request<import('../types').Booking>(`/bookings/${id}/absence`, { method: 'POST', body: JSON.stringify({ reason }) }),
  requestPostPickupCancellation: (id: string, reason?: string) => request<import('../types').PostPickupCancellationRequest>(`/bookings/${id}/post-pickup-cancellation`, { method: 'POST', body: JSON.stringify({ reason }) }),
  acceptPostPickupCancellation: (id: string, requestId: string, destination: object) => request<import('../types').Booking>(`/bookings/${id}/post-pickup-cancellation/${requestId}/accept`, { method: 'POST', body: JSON.stringify({ destination }) }),
  rejectPostPickupCancellation: (id: string, requestId: string) => request(`/bookings/${id}/post-pickup-cancellation/${requestId}/reject`, { method: 'POST' }),
  paymentFallback: (id: string) => request<{ clientSecret: string }>(`/bookings/${id}/payment-fallback`, { method: 'POST' }),
  confirmPaymentFallback: (id: string) => request<import('../types').Booking>(`/bookings/${id}/payment-fallback/confirmed`, { method: 'POST' }),
};
